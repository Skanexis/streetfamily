...
- Заполненный корневой `.env.deploy` по инструкции [`SUPABASE_SETUP_RU.md`](SUPABASE_SETUP_RU.md).

Проект использует один файл `.env.deploy`. `VITE_SUPABASE_PUBLISHABLE_KEY` попадает в браузерную сборку по назначению. Никогда не добавляйте `.env.deploy`, `service_role` key или Telegram bot token в Git.
...
Перед `git commit` посмотрите вывод `git status`: файла `.env.deploy` и секретов там быть не должно. На GitHub сначала создайте пустой репозиторий без README, чтобы первый push прошел без конфликта.
...
cd /opt/apps
git clone https://github.com/Skanexis/streetfamily.git
cd streetfamily
```
...
## 5. Настроить единый `.env.deploy` и свободный порт
...
sudo certbot --nginx -d streetfamily.net
sudo certbot renew --dry-run
```

Замените `street.example.com` своим доменом. Certbot добавит HTTPS в конфигурацию именно этого Nginx-сайта.

После появления HTTPS-адреса укажите его в `.env.deploy` как `TELEGRAM_MINI_APP_URL`, а загрузку Supabase Functions, secrets и webhook выполните по [`SUPABASE_SETUP_RU.md`](SUPABASE_SETUP_RU.md).
...
Functions и заново откройте Mini App исключённым аккаунтом: при следующемul
...
- `.env.deploy.example` является шаблоном настроек, реальный `.env.deploy` остается только локально и на VPS.
