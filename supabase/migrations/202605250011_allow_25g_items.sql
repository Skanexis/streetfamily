-- Allow individual 25 g products; service minimums are enforced on total cart weight.
...
declare
  ...
  v_display text := '#ORD-' || upper(substr(replace(v_order::text, '-', ''), 1, 8));
...
begin
  ...
  return jsonb_build_object(
    ...
    'balance', v_wallet.points, 'disclaimer', ''
  );
end $$;
