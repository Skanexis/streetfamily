alter table public.estrazioni
  add column if not exists viber_required boolean not null default false,
  add column if not exists viber_channel_url text not null default '';

update public.estrazioni
set viber_required = false
where viber_required is null;

update public.estrazioni
set viber_channel_url = ''
where viber_channel_url is null;

alter table public.estrazioni
  alter column viber_required set default false,
  alter column viber_required set not null,
  alter column viber_channel_url set default '',
  alter column viber_channel_url set not null;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conrelid = 'public.estrazioni'::regclass
      and conname = 'estrazioni_viber_channel_url_check'
  ) then
    alter table public.estrazioni
      add constraint estrazioni_viber_channel_url_check
      check (viber_channel_url = '' or viber_channel_url ~* '^https?://');
  end if;
end $$;
...
returns jsonb language sql stable security definer set search_path = public as $$
  ...
  select jsonb_build_object(
    'estrazione', jsonb_build_object(
      ...
      'viber_required', draw.viber_required,
      'viber_channel_url', draw.viber_channel_url,
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'viber_required', e.viber_required,
    'viber_channel_url', e.viber_channel_url,
...
drop function if exists public.admin_upsert_estrazione(uuid, text, integer, integer, integer, integer, boolean, text, text, integer, integer, integer, integer, boolean, text);
drop function if exists public.admin_upsert_estrazione(uuid, text, integer, integer, integer, integer, boolean, text, text, integer, boolean, text, integer, integer, integer);
create or replace function public.admin_upsert_estrazione(
  ...
  p_viber_required boolean default false,
  p_viber_channel_url text default ''
)
...
begin
  ...
  v_viber_required := coalesce(p_viber_required, false);
  v_viber_url := trim(coalesce(p_viber_channel_url, ''));
  ...
  if v_viber_url <> '' and (char_length(v_viber_url) > 700 or v_viber_url !~* '^https?://') then
    raise exception 'ESTR_VIBER_URL_INVALID';
  end if;
  if v_viber_required and v_viber_url = '' then
    raise exception 'ESTR_VIBER_URL_REQUIRED';
  end if;
  ...
  if p_id is null then
    insert into public.estrazioni (
      ...
      viber_required, viber_channel_url, created_by, updated_by
    ) values (
      ...
      v_viber_required, v_viber_url, auth.uid(), auth.uid()
    )
    ...
    set title = trim(p_title),
      ...
      viber_required = v_viber_required,
      viber_channel_url = v_viber_url,
      updated_by = auth.uid()
    where id = p_id
    returning id into v_id;
  end if;
...
grant execute on function public.admin_upsert_estrazione(uuid, text, integer, integer, integer, integer, boolean, text, text, integer, integer, integer, integer, boolean, text) to authenticated;
...
revoke execute on function public.admin_upsert_estrazione(uuid, text, integer, integer, integer, integer, boolean, text, text, integer, integer, integer, integer, boolean, text) from public, anon;
