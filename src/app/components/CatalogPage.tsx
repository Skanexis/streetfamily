...
function Detail({ product, feedback, onClose, addToCart }: { product?: Product; feedback: Feedback[]; onClose: () => void; addToCart: AddFn }) {
  ...
  return <motion.div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-0 md:p-6" style={{ background: 'rgba(8,12,14,.88)' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}>
    <motion.div className="w-full md:max-w-2xl max-h-[92vh] overflow-y-auto p-6" style={panel} onClick={event => event.stopPropagation()} initial={{ y: 30 }} animate={{ y: 0 }}>
      <button onClick={onClose} className="float-right"><X size={20} /></button>
      <span style={kicker}>{product.category}</span><h2 style={{ fontFamily: 'Space Grotesk', fontWeight: 700, fontSize: 30, margin: '8px 0' }}>{product.name}</h2>
      <p style={{ color: 'rgba(245,245,245,.6)', marginBottom: 22 }}>{product.description}</p>
      <div className="grid grid-cols-2 md:grid-cols-5 gap-2 mb-6">{product.variants.filter(item => item.available).map(variant => <button key={variant.id} onClick={() => setSelected(variant)} className="p-3" style={{ border: `1px solid ${selected.id === variant.id ? '#D7FE55' : 'rgba(126,156,168,.2)'}`, background: selected.id === variant.id ? 'rgba(215,254,85,.06)' : '#080C0E' }}><strong style={{ display: 'block' }}>{variant.unitAmount}</strong><span style={{ fontSize: 11 }}>units</span><div style={{ color: '#D7FE55', marginTop: 5 }}>EUR {variant.price}</div></button>)}</div>
      <div className="p-3 mb-5" style={{ background: 'rgba(215,254,85,.06)', color: '#D7FE55' }}>+{selected.tokenAward} gettoni dopo uno scenario completato</div>
      <button onClick={add} className="w-full py-4 flex justify-center gap-2" style={{ background: '#D7FE55', color: '#080C0E', fontWeight: 700 }}><ShoppingCart size={18} />{added ? 'Aggiunto' : 'Aggiungi alla richiesta demo'}</button>
      <h3 style={{ fontFamily: 'Space Grotesk', fontSize: 20, fontWeight: 700, margin: '30px 0 12px' }}>Feedback community pubblicati</h3>
