...
export function CatalogPage({ products, feedback, addToCart, selectedProductId, onProductSelect }: Props) {
  ...
  return <div className="min-h-screen px-4 md:px-8 py-10" style={{ paddingTop: 100 }}>
    <div className="max-w-6xl mx-auto">
      <div className="sf-kicker mb-4">Catalogo dimostrativo</div>
      <h1 style={{ fontFamily: 'Space Grotesk', fontWeight: 700, fontSize: 36 }}>Collezioni dimostrative</h1>
      <p style={{ color: 'rgba(245,245,245,.58)', margin: '8px 0 28px' }}>Prezzi per peso e scelta personalizzata in grammi. Nessuna vendita o consegna reale.</p>
...
function Card({ product, onClick }: { product: Product; onClick: () => void }) {
  return <motion.button whileHover={{ y: -3 }} onClick={onClick} className="text-left overflow-hidden" style={panel}>
    {product.img ? <img src={product.img} alt={product.name} className="w-full object-cover" style={{ height: 176, background: '#182226' }} /> : <div className="flex items-center justify-center" style={{ height: 176, background: '#182226', color: 'rgba(245,245,245,.35)', fontFamily: 'Orbitron', fontSize: 11 }}>MEDIA DIMOSTRATIVO</div>}
    <div className="p-4"><span style={kicker}>{product.category}</span><h2 style={{ fontFamily: 'Space Grotesk', fontWeight: 700, margin: '5px 0 12px' }}>{product.name}</h2><div className="flex justify-between"><span style={{ color: 'rgba(245,245,245,.55)', fontSize: 12 }}>da 50 g</span><strong style={{ color: '#D7FE55' }}>EUR {formatPrice(product.startingPrice)}</strong></div></div>
  </motion.button>
}
  ...
  return <motion.div className="fixed inset-0 flex items-end md:items-center justify-center p-0 md:p-6" style={{ zIndex: 70, background: 'rgba(8,12,14,.88)' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}>
    <motion.div className="w-full md:max-w-2xl overflow-y-auto p-4 md:p-6" style={{ ...panel, maxHeight: 'calc(100dvh - env(safe-area-inset-top, 0px) - env(safe-area-inset-bottom, 0px) - 12px)', paddingBottom: 'max(18px, env(safe-area-inset-bottom, 0px))' }} onClick={event => event.stopPropagation()} initial={{ y: 30 }} animate={{ y: 0 }}>
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-2 mb-3">{availableVariants.map(variant => {
        ...
        return <button key={variant.id} disabled={!selectable} onClick={() => setCustomGrams(String(variant.unitAmount))} className="py-2 px-1" style={{ minHeight: 64, opacity: selectable ? 1 : .62, border: `1px solid ${selected ? '#D7FE55' : 'rgba(126,156,168,.2)'}`, background: selected ? 'rgba(215,254,85,.06)' : '#080C0E' }}><strong style={{ display: 'block', fontSize: 17 }}>{variant.unitAmount}g</strong><div style={{ color: '#D7FE55', marginTop: 2, fontSize: 13 }}>EUR {formatPrice(variant.price)}</div>{!selectable && <span style={{ fontSize: 9, color: 'rgba(245,245,245,.58)' }}>solo prezzo</span>}</button>
      })}</div>
      ...
      <div className="px-3 py-2 mb-3" style={{ background: 'rgba(215,254,85,.06)', color: '#D7FE55', fontSize: 13 }}>+{tokenAward} gettoni dopo uno scenario completato</div>
      <button onClick={add} disabled={!validWeight} className="w-full py-3 flex justify-center gap-2" style={{ background: '#D7FE55', color: '#080C0E', fontWeight: 700, opacity: validWeight ? 1 : .5 }}><ShoppingCart size={18} />{added ? 'Aggiunto' : 'Aggiungi alla richiesta dimostrativa'}</button>
