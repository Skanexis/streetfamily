...
type Step = 'cart' | 'scenario' | 'details' | 'summary' | 'kyc' | 'success'
const scenarios: { key: ScenarioType; label: string; Icon: typeof MapPin }[] = [
  { key: 'meetup', label: 'Meet up simulato', Icon: MapPin },
  { key: 'delivery_zone', label: 'Delivery zone simulata', Icon: Truck },
  { key: 'delivery_italia', label: 'Delivery Italia simulata', Icon: Truck },
]
...
export function CartDrawer({ open, onClose, cart, removeFromCart, tokens, serviceAreas, firstOrder, kycStatus, onKycChanged, onSubmit, onComplete }: Props) {
  ...
  const reset = () => {
    setStep('cart'); setScenarioType('meetup'); setCity(''); setStreet(''); setTokensToReserve(0); setError(''); setResult(null)
  }
  ...
  const continueDetails = () => {
    ...
    if (totalUnits > 1000) return setError('La prima versione supporta al massimo 1000 units per richiesta demo.')
    if (!city.trim()) return setError('Seleziona o inserisci una citta demo.')
    if ((scenarioType !== 'meetup') && !street.trim()) return setError('Inserisci una via di esempio per lo scenario demo.')
    if (min && totalUnits < min) return setError(`Questo scenario richiede almeno ${min} units.`)
  ...
  const confirm = async () => {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : 'Impossibile inviare la richiesta demo.')
        ...
        <motion.div className="fixed right-0 top-0 bottom-0 flex flex-col w-full sm:max-w-md" style={{ zIndex: 71, paddingBottom: 'env(safe-area-inset-bottom, 0px)', background: '#080C0E', borderLeft: '1px solid rgba(126,156,168,.2)' }} initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }}>
          ...
          <div className="flex-1 overflow-y-auto p-5">
            {step === 'cart' && <div className="flex flex-col gap-3">
              {cart.length === 0 && <p style={muted}>Il carrello demo e vuoto.</p>}
              {cart.map(item => <div key={item.id} className="flex gap-3 p-3" style={panel}>
                {item.img ? <img src={item.img} alt={item.name} className="object-cover" style={{ width: 52, height: 52 }} /> : <div className="flex items-center justify-center" style={{ width: 52, height: 52, background: '#182226', color: 'rgba(245,245,245,.35)', fontSize: 9 }}>DEMO</div>}
              ...
              {cart.length > 0 && <div className="p-4" style={accent}><Star size={14} className="inline mr-2" />{totalUnits} units / +{tokenReward} gettoni soltanto dopo completamento demo</div>}
            </div>}
            {step === 'scenario' && <div className="flex flex-col gap-3">
              <p style={muted}>Seleziona unicamente uno scenario dimostrativo.</p>
              {scenarios.map(({ key, label, Icon }) => <button key={key} onClick={() => chooseScenario(key)} className="p-5 flex gap-4 text-left" style={panel}><Icon style={{ color: '#D7FE55' }} /><div><strong>{label}</strong><div style={muted}>Nessun servizio reale verra richiesto</div></div></button>)}
            </div>}
            {step === 'details' && <div className="flex flex-col gap-4">
              <p style={accent}>Usa solo dati di esempio: questo e un test di interfaccia.</p>
              <label style={muted}>Citta demo</label>
              {scenarioType === 'delivery_italia'
                ? <input value={city} onChange={e => setCity(e.target.value)} placeholder="Citta demo" style={input} />
                : <select value={city} onChange={e => setCity(e.target.value)} style={input}><option value="">Seleziona citta</option>{selectedAreas.map(area => <option key={area.id} value={area.city}>{area.city} - min {area.minimumUnits} units</option>)}</select>}
              {scenarioType !== 'meetup' && <><label style={muted}>Via di esempio</label><input value={street} onChange={e => setStreet(e.target.value)} placeholder="Via demo" style={input} /></>}
              {error && <p style={{ color: '#EF4444' }}>{error}</p>}
            </div>}
            {step === 'summary' && <div className="flex flex-col gap-4">
              <div className="p-4" style={panel}><div style={muted}>SCENARIO DEMO</div><strong>{scenarios.find(item => item.key === scenarioType)?.label}</strong><div>{city}{street && ` / ${street}`}</div></div>
              <Line label="Subtotal simulato" value={subtotal} />
              {surcharge > 0 && <Line label="Surcharge simulato" value={surcharge} />}
              <label style={muted}>Usa gettoni come credit demo (saldo: {tokens})</label>
              ...
              {tokens >= 100 && <p style={accent}>Saldo massimo raggiunto: usa almeno 1 gettone per inviare una nuova richiesta demo.</p>}
              <Line label="Totale simulato" value={Math.max(subtotal + surcharge - tokensToReserve, 0)} strong />
              {error && <p style={{ color: '#EF4444' }}>{error}</p>}
            </div>}
            {step === 'kyc' && <><h2 style={{ fontFamily: 'Space Grotesk', fontSize: 21, fontWeight: 700 }}>Verifica identita</h2><p style={muted}>Richiesta prima della prima simulazione.</p><KycCapture status={kycStatus} onChanged={onKycChanged} /></>}
            {step === 'success' && result && <div className="text-center py-12"><Check size={50} style={{ color: '#D7FE55', margin: '0 auto 16px' }} /><h2 style={{ color: '#D7FE55', fontFamily: 'Orbitron' }}>RICHIESTA DEMO INVIATA</h2><p>{result.displayId}</p><p style={muted}>{result.disclaimer}</p><div className="p-4 mt-6" style={panel}>EUR {result.simulatedTotal} / {result.totalUnits} units<br />+{result.tokensOnComplete} gettoni dopo completamento</div></div>}
          </div>
          {step !== 'success' && cart.length > 0 && <footer className="p-5" style={{ borderTop: '1px solid rgba(245,245,245,.08)' }}>
            ...
            {step === 'summary' && requiresKyc && <Action onClick={() => setStep('kyc')} label="Verifica identita per continuare" />}
            {step === 'summary' && !requiresKyc && <Action onClick={confirm} label={saving ? 'Invio...' : 'Conferma richiesta demo'} disabled={saving} />}
            {step === 'kyc' && kycStatus.status === 'approved' && <Action onClick={confirm} label="Conferma richiesta demo" disabled={saving} />}
          </footer>}
