...
const statusLabel = { submitted: 'Inviato', processing: 'In revisione', completed: 'Prova completata', cancelled: 'Annullato' }
  ...
  return <div className="min-h-screen px-4 md:px-8 py-10" style={{ paddingTop: 100 }}>
    <div className="max-w-2xl mx-auto">
      ...
      <section className="p-6 mb-7" style={panel}>
        ...
        {user.tokens >= 100 && <p className="p-3 mt-5" style={notice}>Limite gettoni raggiunto: nella prossima richiesta dimostrativa dovrai usarne almeno uno.</p>}
      </section>
...
function OrderCard({ order, onChanged }: { order: TestOrder; onChanged: () => Promise<void> }) {
  ...
  const send = async () => {
    ...
    catch (caught) { setError(caught instanceof Error ? caught.message : 'Recensione non salvata.') }
  }
  return <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="p-4" style={panel}>
    ...
    <p style={small}>{order.totalUnits} g / EUR {order.total} simulati</p>
