    ...
    <div className="max-w-2xl mx-auto">
      ...
      <section className="p-6 mb-7" style={panel}>
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <Stat Icon={Package} value={user.completedOrders} label="Completed" />
        </div>
        {user.tokens >= 100 && <p className="p-3 mt-5" style={notice}>Limite gettoni raggiunto: nella prossima richiesta demo dovrai usarne almeno uno.</p>}
      </section>
...
function OrderCard({ order, onChanged }: { order: TestOrder; onChanged: () => Promise<void> }) {
  ...
  const send = async () => {
    ...
    catch (caught) { setError(caught instanceof Error ? caught.message : 'Feedback non salvato.') }
  }
